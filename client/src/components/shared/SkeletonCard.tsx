...
export default function SkeletonCard({ className = '', lines = 3 }: SkeletonCardProps) {
  return (
    <div className={`glass-card rounded-2xl p-5 ${className}`}>
      <div className="h-4 w-2/3 bg-surface-container-high rounded mb-3 skeleton-shimmer" />
      {Array.from({ length: lines }).map((_, i) => (
        <div
          key={i}
          className="h-3 bg-surface-container-high rounded mb-2 last:mb-0 skeleton-shimmer"
          style={{ width: `${90 - i * 15}%` }}
        />
      ))}
    </div>
